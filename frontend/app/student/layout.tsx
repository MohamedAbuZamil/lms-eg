...
import { LayoutDashboard, BookOpen, Store, Wallet, Bell, Settings, LogOut, Menu, X } from 'lucide-react';
import { MOCK_STUDENT } from '@/lib/mock-data';

const NAV = [
  { label: 'الرئيسية',  href: '/student',               icon: LayoutDashboard },
  { label: 'كورساتي',   href: '/student/courses',       icon: BookOpen        },
  { label: 'المتجر',    href: '/student/marketplace',   icon: Store           },
  { label: 'رصيدي',     href: '/student/balance',       icon: Wallet          },
...
function SidebarContent({
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Brand */}
      <div className="px-5 py-4 border-b border-lms-50 flex items-center gap-3">
        <LmsLogo size={34} />
        <span className="font-bold text-lg text-foreground">LMS-EG</span>
      </div>

      {/* User card */}
      <div className="px-4 py-3 border-b border-lms-50">
        <div className="flex items-center gap-3 p-2.5 rounded-2xl bg-lms-50">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-lms-500 font-medium">طالب</p>
          </div>
        </div>
      </div>
      ...
      {/* Logout */}
      <div className="px-3 pb-4 pt-2 border-t border-lms-50">
        <button onClick={onLogout}
          className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:bg-red-50 hover:text-red-500 transition-all w-full">
  ...
  return (
    <div className="min-h-screen bg-[#F5F0FF]" dir="rtl">
      ...
      <main className="lg:mr-64 min-h-screen pt-14 lg:pt-0">
        <div className="max-w-5xl mx-auto px-4 lg:px-6 py-5 lg:py-6">{children}</div>
      </main>
    </div>
  );
}
