...
import { MOCK_USER, MOCK_TEACHER_STATS } from '@/lib/mock-data';
import { roleLabel } from '@/lib/utils';
import { Mail, Phone, Shield, LogOut, User, Star } from 'lucide-react';
...
export default function TeacherSettingsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in-up max-w-xl mx-auto">
      ...
      <div className="bg-white rounded-3xl border border-lms-100 shadow-elevated overflow-hidden">
        <div className="bg-lms-gradient px-6 py-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="text-lms-200 text-sm mt-0.5">{roleLabel(me.role)}</p>
          </div>
          <div className="flex items-center gap-3 mt-1">
            <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/20 text-white text-xs font-semibold">
              <Star size={12} className="fill-amber-300 text-amber-300" />{MOCK_TEACHER_STATS.rating}
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white/20 text-white text-xs font-semibold">
              {MOCK_TEACHER_STATS.totalStudents.toLocaleString('ar-EG')} طالب
            </span>
            <span className="px-3 py-1.5 rounded-xl bg-white/20 text-white text-xs font-semibold">
              {MOCK_TEACHER_STATS.totalCourses} كورس
            </span>
          </div>
        </div>
        ...
        <div className="p-5 space-y-3">
          {[
            ...
            { icon: <Shield size={15} />, label: 'نوع الحساب',        value: roleLabel(me.role)  },
          ].map((row) => (
      ...
      <button onClick={() => { logout(); router.push('/teacher/login'); }}
