...
const INITIAL = [
  { id: '1', title: 'طالب جديد',       body: 'انضم أحمد محمد إلى Course 1',             time: 'منذ ساعة',    read: false },
  { id: '2', title: 'تسليم واجب',       body: 'سارة إبراهيم سلّمت واجب Unit 7',         time: 'منذ 2 ساعة',  read: false },
  { id: '3', title: 'اكتمل الامتحان',   body: '5 طلاب أجروا امتحان Active & Passive',   time: 'منذ 5 ساعات', read: true  },
  { id: '4', title: 'طلب تسجيل',        body: 'محمود حسن طلب الاشتراك في Course 2',     time: 'أمس',          read: true  },
];
    ...
    <div className="space-y-6 animate-fade-in-up max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-2 px-3 py-2 rounded-xl bg-lms-gradient text-white text-sm font-semibold hover:shadow-glow-purple transition-shadow">
            <Plus size={14} />إرسال إشعار
          </button>
          {unread > 0 && (
            <button onClick={() => setNotifs((p) => p.map((n) => ({ ...n, read: true })))}
              className="flex items-center gap-2 px-3 py-2 rounded-xl bg-lms-50 border border-lms-200 text-lms-700 text-sm font-medium hover:bg-lms-100 transition-colors">
              <Check size={14} />تعليم الكل
            </button>
          )}
        </div>
      </div>
